...
| `__init__.py` | Makes this folder an importable package (`ingestion.downloader`, `ingestion.pdf_loader`). |

Corresponding tests live in `tests/test_downloader.py` and `tests/test_pdf_loader.py`, not in this folder — see [Running the tests](#running-the-tests).
