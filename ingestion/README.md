...
- **Rate limiting.** `download()` sleeps 3 seconds after each request, and the CLI sleeps 3 seconds between successive `--query` topics too, per arXiv's own usage guideline. Don't remove this when scripting bulk downloads.
- **429 (rate limited) is retried automatically**, up to 4 attempts with exponential backoff (5s, 10s, 20s). This matters most from shared/datacenter IP ranges (cloud VMs), which arXiv appears to rate-limit more aggressively than residential IPs even when this client is already being polite. Any other error status (400, 404, 500...) raises immediately, with no retry.
- **A descriptive `User-Agent`** is set by default (`research-copilot/0.1 ...`), as arXiv's API etiquette asks. An explicitly injected `requests.Session` with its own `User-Agent` already set is left untouched.
